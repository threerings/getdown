...
public class Application
{
    ...
    {
      return "[name=" + name + ", bg=" + background + ", bg=" + backgroundImage + ", pi=" +
          progressImage + ", prect=" + progress + ", pt=" + progressText + ", pb=" + progressBar +
          ", srect=" + status + ", st=" + statusText + ", shadow=" + textShadow + ", err=" +
          installError + ", nrect=" + patchNotes + ", notes=" + patchNotesUrl +
          ", stepPercentages=" + stepPercentages + ", hideProgressText" + hideProgressText +
          ", minShow=" + minShowSeconds + "]";
    }
    ...
    public UpdateInterface(Config config)
    {
      ...
      int defaultBackground = (0.5f < Color.brightness(this.progressText)) ? Color.BLACK
          : Color.WHITE;
      ...
      this.installError = (installError == null) ? "m.default_install_error"
          : MessageUtil.taint(installError);
  ...
  public Resource getPatchResource(String auxgroup)
  {
    ...
    {
      log.warning("Requested patch resource for up-to-date or non-versioned application", "cvers",
          _version, "tvers", _targetVersion);
      return null;
    }
    ...
    {
      log.warning("Failed to create patch resource path", "pfile", pfile, "appbase", _appbase,
          "tvers", _targetVersion, "error", e);
      return null;
    }
  }
  ...
  public Resource getFullResource()
  {
    ...
    {
      log.warning("Failed to create full resource path", "file", file, "appbase", _appbase, "tvers",
          _targetVersion, "error", e);
      return null;
    }
  }
  ...
  public Config init(boolean checkPlatform) throws IOException
  {
    ...
    _codeCacheRetentionDays = config.getInt("code_cache_retention_days", 7);
        ...
        _dockIconPath = config.getString("ui.mac_dock_icon", "../desktop.icns");return config;
    }
  ...
  public boolean haveValidJavaVersion()
  {
    ...
    try
    {
      ...
      log.info("Checking Java version", "current", version, "wantMin", _javaMinVersion, "wantMax",
          _javaMaxVersion);
      ...
      if (_javaExactVersionRequired)
      {
        ...
        {
          log.warning("An exact Java VM version is required.", "current", version, "required",
              _javaMinVersion);
          return false;
        }
      }
    ...
    catch (RuntimeException re)
    {
      ...
      log.warning("Unable to parse VM version, hoping for the best", "error", re, "needed",
          _javaMinVersion);
      return true;
    }
  }
  ...
  public boolean verifyMetadata(StatusDisplay status) throws IOException
  {
    ...
    if (_version == -1)
    {
      ...
      {
        log.warning("Failed to refresh non-versioned digest: " + ioe.getMessage() +
            ". Proceeding...");
      }
    }
    ...
    for (int ii = 0; ii < sizes.length; ii++)
    {
      ...
      exec.execute(new Runnable()
      {
        public void run()
        {
          verifyResource(rsrc, pagg.startElement(index), fAlreadyValid, unpackedAsync,
              toInstallAsync, toDownloadAsync);
          actions.add(new Runnable()
          {
            public void run()
            {
    ...
    while (completed[0] < rsrcs.size())
    {
      ...
      action.run();
        ...
        exec.shutdown();toInstall.addAll(toInstallAsync);
  ...
  private void verifyResource(Resource rsrc, ProgressObserver obs, int[] alreadyValid,
      Set<Resource> unpacked, Set<Resource> toInstall, Set<Resource> toDownload)
  {
  ...
  protected File downloadFile(String path) throws IOException
  {
    ...
    {
      log.warning("Requested to download invalid control file", "appbase", _vappbase, "path", path,
          "error", e);
