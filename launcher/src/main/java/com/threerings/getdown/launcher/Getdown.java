...
public abstract class Getdown extends Thread implements Application.StatusDisplay, RotatingBackgrounds.ImageLoader {
			...
			if (msg == null) {
				...
				if (e instanceof FileNotFoundException) {
					msg = MessageUtil.compose("m.missing_resource", MessageUtil.taint(msg), _ifc.installError);
						...
						msg = MessageUtil.compose("m.ssl_error", System.getProperty("appbase"));
				} else {
	...
	/**
	 * Configures our proxy settings (called by {@link ProxyPanel}) and fires up
	 * the launcher.
	 */
	...
	protected boolean detectProxy() throws IOException {
		...
		// we got through, so we appear not to require a proxy; make a blank
		// proxy config so that
	...
	/**
	 * Downloads and installs (without verifying) any resources that are marked
	 * with a {@code PRELOAD} attribute.
	 * 
	 * @param resources
	 *            the full set of resources from the application (the
	...
	protected void download(Collection<Resource> resources) throws IOException {
		...
		Downloader dl = new HTTPDownloader(_app.proxy) {
			...
			/**
			 * The last percentage at which we checked for another getdown
			 * running, or -1 for not having checked at all.
			 */
	...
	/**
	 * Called to launch the application if everything is determined to be ready
	 * to go.
	 */
	...
	/**
	 * Creates our user interface, which we avoid doing unless we actually have
	 * to update something. NOTE: this happens on the next UI tick, not
	 * immediately.
	 *
	 * @param reinit
	 *            - if the interface should be reinitialized if it already
	 *            exists.
	 */
	...
	/**
	 * Initializes the interface with the current UpdateInterface and
	 * backgrounds.
	 */
	...
	/**
	 * Set the current step, which will be used to globalize per-step
	 * percentages.
	 */
	...
	/**
	 * Updates the status. NOTE: this happens on the next UI tick, not
	 * immediately.
	 */
	...
	/**
	 * Copies the supplied stream from the specified input to the specified
	 * output. Used to copy our child processes stderr and stdout to our own
	 * stderr and stdout.
	 */
